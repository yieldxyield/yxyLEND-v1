...
class Borrowing extends Component {
  ...
  setModelReturned = (txHash) => {
    ...
    const that = this
    setTimeout(() => {
      const YxYObj = { YxYMessage: txHash, YxYType: 'Hash' }
      that.setState(YxYObj)
    })
  }
  ...
  errorReturned = (error) => {
    const lendObj = { basicMessage: null, YxYType: null }
    this.setState(YxYObj)
    this.setState({ loading: false })
    const that = this
    setTimeout(() => {
      const YxYObj = { YxYMessage: error.toString(), newType: 'Error' }
      that.setState(basicObj)
    })
  };
  ...
  render() {
    ...
    let totalBorrowsyxyUSD = '0.0000'
    if(vault && vault.totalBorrowyxyUSD) {
      if(vault.reservePriceyxyUSD) {
        totalBorrowsyxyUSD = (vault.totalBorrowsyxyUSD/(10**26) / vault.reservePriceyxyUSD).toFixed(4)
      } else {
        totalBorrowsyxyUSD = (vault.totalBorrowsyxyUSD/(10**26)).toFixed(4)
      }
    }

    let availableBorrowsyxyUSD = '0.0000'
    if(vault && vault.availableBorrowsyxyUSD) {
      if(vault.reservePriceyxyUSD) {
        availableBorrowsyxyUSD = (vault.availableBorrowsyxyUSD/(10**26) / vault.reservePriceyxyUSD).toFixed(4)
      } else {
        availableBorrowsyxyUSD = (vault.availableBorrowsyxyUSD/(10**26)).toFixed(4)
      }
    }
            ...
            <div className={ classes.container }>
              <div className={ classes.totalsContainer }>
                <div>
                  ...
                  <Typography variant='h2'>$ { vault && vault.totalCollateralyxyUSD ? (vault.totalCollateralyxyUSD/(10**26)).toFixed(4) : '0.0000' }</Typography>
                </div>
                <div>
                  <Typography variant='h3' className={ classes.grey }>Total Liquidity</Typography>
                  <Typography variant='h2'>$ { vault && vault.availableBorrowsyxyUSD ? (vault.availableBorrowsyxyUSD/(10**26)).toFixed(4) : '0.0000' }</Typography>
                </div>
                <div>
                  <Typography variant='h3' className={ classes.grey }>Total Borrowed</Typography>
                  <Typography variant='h2'>{ vault.borrowSymbol === '$' ? vault.borrowSymbol : '' } { totalBorrowsyxyUSD } { vault.borrowSymbol !== '$' ? vault.borrowSymbol : '' }</Typography>
                </div>
              </div>
            </div>
        }
            ...
            <div className={ classes.amountContainer }>
              ...
              <div className={ classes.valContainer }>
                <div className={ classes.balances }>
                  <Typography variant='h4' onClick={ () => { this.setAmount('amount', availableBorrowsyxyUSD) } } className={ classes.value } noWrap>{ 'Balance: ' + (vault.borrowSymbol === '$' ? vault.borrowSymbol : '') + availableBorrowsyxyUSD + (vault.borrowSymbol !== '$' ? vault.borrowSymbol : '') } </Typography>
                </div>
                <div>
                  <TextField
                    fullWidth
  ...
  renderYxY = () => {
    ...
    return <YxY type={ YxYType } message={ YxYMessage } open={true}/>
  };
